...
export const ProfileTab: React.FC<ProfileTabProps> = ({
  ...
  const [name, setName] = useState(userProfile.name || 'Sky');
  ...
  const [bio, setBio] = useState(
    userProfile.bio ||
      'Creative night owl, loves quiet coffee shops, good music, and deep late-night conversations.'
  );
          ...
          <div className="p-5 rounded-3xl bg-gradient-to-br from-[#005baa]/25 via-[#241b22] to-[#241b22] border border-[#005baa]/50 flex items-center justify-between gap-4">
            <div className="space-y-1">
              <h2 className="text-sm font-bold text-white flex items-center gap-1.5">
                ...
                <span>{language === 'my' ? 'Barrychat ကို ထောက်ပံ့လှူဒါန်းရန်' : 'Support & Donate (KBZPay)'}</span>
              </h2>
              <p className="text-xs text-[#a0909c]">
                {language === 'my'
                  ? 'အက်ပ်ကို အခမဲ့ ဆက်လက်အသုံးပြုနိုင်ရန် KBZPay ဖြင့် ထောက်ပံ့လှူဒါန်းနိုင်ပါသည်။'
                  : 'Support Barrychat development and keep it 100% free via KBZPay QR.'}
              </p>
            </div>
