        ...
        {filtered.length === 0 ? (
          <div className="h-64 flex flex-col items-center justify-center text-center p-6 text-[#a0909c] space-y-3">
            ...
            <p className="text-xs max-w-xs">
              {searchQuery
                ? 'Try searching with a different name or trait.'
                : 'Tap the + button at the top to create your first companion!'}
            </p>
          </div>
        ) : (
          filtered.map((char) => {
              ...
              >
                ...
                <div className="flex items-center gap-1 opacity-80 sm:opacity-0 group-hover:opacity-100 transition-opacity shrink-0">
                  ...
                  <button
                    onClick={(e) => {
                      e.stopPropagation();
                      if (window.confirm(`Delete ${char.name}? All messages will be permanently deleted.`)) {
