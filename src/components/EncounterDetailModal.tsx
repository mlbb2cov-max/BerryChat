    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-black/80 backdrop-blur-sm animate-fade-in">
      ...
      >
        ...
        <div className="p-4 sm:p-5 border-b border-[#3d2b38] flex items-center justify-between bg-[#1f161d] shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="px-2 py-0.5 rounded-full bg-[#ff85a2]/15 text-[#ff85a2] border border-[#ff85a2]/30 text-[10px] font-semibold flex items-center gap-1">
                  <Lock className="w-2.5 h-2.5" />
                  {language === 'my' ? 'ဖတ်ရှုရန်သာ' : 'Ended Meet • Read-Only'}
                </span>
              </div>
              <div className="flex items-center gap-3 text-xs text-[#a0909c] mt-0.5">
          ...
          <button
            onClick={onClose}
            className="p-2 rounded-xl text-[#a0909c] hover:text-white hover:bg-[#3d2b38] transition-colors"
            aria-label="Close"
          >
            <X className="w-5 h-5" />
          </button>
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-5 space-y-4">
          <div className="text-center py-1">
            <span className="px-3 py-1 rounded-full bg-[#1a1218] border border-[#3d2b38] text-[11px] text-[#a0909c] inline-flex items-center gap-1.5">
          ...
          {encounter.summary && (
            <div className="p-4 rounded-2xl bg-gradient-to-br from-[#ff85a2]/10 to-[#ff5a8a]/5 border border-[#ff85a2]/30 text-xs text-stone-300">
              <div className="font-bold text-[#ff85a2] flex items-center gap-1.5 mb-1 text-sm">
                ...
                <span>
                  {language === 'my' ? 'တွေ့ဆုံမှု အနှစ်ချုပ်' : 'Encounter Summary & Bonding'}
                </span>
              </div>
        ...
        <div className="p-4 border-t border-[#3d2b38] bg-[#1f161d] flex items-center justify-between shrink-0">
          <div className="text-xs text-[#a0909c] flex items-center gap-1.5">
            <Lock className="w-3.5 h-3.5 text-[#ff85a2]" />
            <span>
              {language === 'my'
                ? 'နောက်တစ်ကြိမ် တွေ့ဆုံရန် Meet Mode ခလုတ်ကို နှိပ်ပါ'
                : 'To meet in person again, tap Meet Mode'}
            </span>
          </div>
          <button
            onClick={onClose}
            className="px-5 py-2 rounded-xl bg-[#2e222c] hover:bg-[#3d2b38] text-white text-xs font-semibold border border-[#3d2b38] transition-colors"
          >
